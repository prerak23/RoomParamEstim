...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar, mse_loss):
    ...
    adcc = np.zeros((1, 6))  # Total 14 Values
    ...
    rt60_125_ ,rt60_250_ ,rt60_500_ ,rt60_1000_ ,rt60_2000_ ,rt60_4000_ = 0,0,0,0,0,0

    #ab_125_ ,ab_250_ ,ab_500_ ,ab_1000_ ,ab_2000_ ,ab_4000_ = 0,0,0,0,0,0

    for batch_idx, sample_batched in enumerate(train_loader):
        data, surface, volume, absorption, rt60 = sample_batched['bnsample'].float(), sample_batched['surface'].float().to(device="cuda"), sample_batched['volume'].float().to(device="cuda"), sample_batched['absorption'].float().to(device="cuda"), sample_batched['rt60'].float().to(device="cuda")
        ...
        output = model(x_1, ipd_ild)

        #target_= torch.cat((surface, volume, rt60, absorption), axis=1)



        #target_ = torch.cat((surface, volume), axis=1)

        target_=rt60.reshape(128,6)
        ...
        '''
        for i in range(6):
            rt60_loss += NLLloss(rt60[:, i], mean[:, 2 + i], variance[:, 2 + i])
            absorp_loss += NLLloss(absorption[:, i], mean[:, 8 + i], variance[:, 8 + i])
        ...
        #surface_mse += float(pred(output[:, 0] , surface.view(128)).item())

        #volume_mse += float(pred(output[:, 0] , volume.view(128)).item())

        
        rt60_125_ += float(pred(output[:, 0], rt60[:, 0]).item())
        rt60_250_ += float(pred(output[:, 1], rt60[:, 1]).item())
        rt60_500_ += float(pred(output[:, 2], rt60[:, 2]).item())
        rt60_1000_ += float(pred(output[:, 3], rt60[:, 3]).item())
        rt60_2000_ += float(pred(output[:, 4], rt60[:, 4]).item())
        rt60_4000_ += float(pred(output[:, 5], rt60[:, 5]).item())
        
        '''
        ab_125_ += float(pred(output[:, 8], absorption[:, 0]).item())
        ab_250_ += float(pred(output[:, 9], absorption[:, 1]).item())
        ab_500_ += float(pred(output[:, 10], absorption[:, 2]).item())
        ab_1000_ += float(pred(output[:, 11], absorption[:, 3]).item())
        ab_2000_ += float(pred(output[:, 12], absorption[:, 4]).item())
        ab_4000_ += float(pred(output[:, 13], absorption[:, 5]).item())
        '''
        ...
        #total = torch.cat((surface, volume), axis=1)

        #track_var = np.concatenate((track_var, variance.detach().cpu().clone().numpy().reshape(128, 14)), axis=0)

        if epoch == 0:
            adcc = np.concatenate((adcc, rt60.detach().cpu().clone().numpy().reshape(128, 6)), axis=0)
        ...
        if batch_idx % 100 == 99:
            ...
            batch_loss_ar.append((surface_mse / 100, volume_mse / 100, (rt60_125_) / 100, (rt60_250_) / 100,
                                  ...
                                  (ab_4000_) / 100))
            '''
            batch_loss_ar.append(((rt60_125_) / 100, (rt60_250_) / 100, (rt60_500_)/100,(rt60_1000_)/100,(rt60_2000_)/100,(rt60_4000_)/100))
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar,mse_loss):
    model.eval()
    #surface_loss_mse, volume_loss_mse = 0,0
    val_loss = 0

    rt60_loss_mse_125 ,rt60_loss_mse_250 ,rt60_loss_mse_500 ,rt60_loss_mse_1000 ,rt60_loss_mse_2000 ,rt60_loss_mse_4000 = 0,0,0,0,0,0

    #absorp_loss_mse_125 ,absorp_loss_mse_250 ,absorp_loss_mse_500 ,absorp_loss_mse_1000 ,absorp_loss_mse_2000 ,absorp_loss_mse_4000 = 0,0,0,0,0,0

    local_data_sp = np.zeros([1, 14])  # 42 #44

    for batch_idx, sample_batched in enumerate(train_loader):

        data, surface, volume, absorption, rt60, rm, vp = sample_batched['bnsample'].float(), sample_batched['surface'].float().to(device="cuda"), sample_batched['volume'].float().to(device="cuda"), sample_batched['absorption'].float().to(device="cuda"), sample_batched['rt60'].float().to(device="cuda"), sample_batched['room'].to(device="cuda"), sample_batched['vp'].to(device="cuda")

        #surface_loss_mse_t ,volume_loss_mse_t = 0,0

        rt60_loss_mse_125_t ,rt60_loss_mse_250_t ,rt60_loss_mse_500_t ,rt60_loss_mse_1000_t ,rt60_loss_mse_2000_t ,rt60_loss_mse_4000_t = 0,0,0,0,0,0

        #absorp_loss_mse_125_t ,absorp_loss_mse_250_t ,absorp_loss_mse_500_t ,absorp_loss_mse_1000_t ,absorp_loss_mse_2000_t ,absorp_loss_mse_4000_t = 0,0,0,0,0,0
        ...
        #target_1 = torch.cat((surface, volume, rt60, absorption), axis=1)
        #target_1 = torch.cat((surface, volume), axis=1)
        
        target_1=rt60.reshape(128,6) #rt60.reshape(128,6)

        val_loss_t=mse_loss(output,target_1)

        val_loss = float(val_loss_t.item()) + val_loss

        #surface_loss_mse_t += pred(output[:, 0] , surface.view(128))

        #volume_loss_mse_t += pred(output[:, 0] , volume.view(128))

        
        rt60_loss_mse_125_t += pred(output[:, 0], rt60[:, 0])
        rt60_loss_mse_250_t += pred(output[:, 1], rt60[:, 1])
        rt60_loss_mse_500_t += pred(output[:, 2], rt60[:, 2])
        rt60_loss_mse_1000_t += pred(output[:, 3], rt60[:, 3])
        rt60_loss_mse_2000_t += pred(output[:, 4], rt60[:, 4])
        rt60_loss_mse_4000_t += pred(output[:, 5], rt60[:, 5])
        
        '''
        absorp_loss_mse_125_t += pred(output[:, 8], absorption[:, 0])
        absorp_loss_mse_250_t += pred(output[:, 9], absorption[:, 1])
        absorp_loss_mse_500_t += pred(output[:, 10], absorption[:, 2])
        absorp_loss_mse_1000_t += pred(output[:, 11], absorption[:, 3])
        absorp_loss_mse_2000_t += pred(output[:, 12], absorption[:, 4])
        absorp_loss_mse_4000_t += pred(output[:, 13], absorption[:, 5])
        '''
        target_ = torch.cat((rt60.reshape(128,6), rm.reshape(128, 1), vp.reshape(128, 1)),axis=1)  # absorption , rt60

        #output = torch.cat((mean, variance), axis=1)

        ass = np.concatenate((output.detach().cpu().clone().numpy().reshape(128, 6),
                              target_.detach().cpu().clone().numpy().reshape(128, 8)), axis=1)
        ...
        del val_loss_t,rt60_loss_mse_125_t, rt60_loss_mse_250_t, rt60_loss_mse_500_t, rt60_loss_mse_1000_t, rt60_loss_mse_2000_t, rt60_loss_mse_4000_t
        
        #absorp_loss_mse_125_t, absorp_loss_mse_250_t, absorp_loss_mse_500_t, absorp_loss_mse_1000_t, absorp_loss_mse_2000_t, absorp_loss_mse_4000_t
        ...
        if batch_idx % 50 == 49:
            ...
            '''
            absorp_125_acc = (absorp_loss_mse_125 / 100)

            absorp_250_acc = (absorp_loss_mse_250 / 100)

            absorp_500_acc = (absorp_loss_mse_500 / 100)
            absorp_1000_acc = (absorp_loss_mse_1000 / 100)
            absorp_2000_acc = (absorp_loss_mse_2000 / 100)
            absorp_4000_acc = (absorp_loss_mse_4000 / 100)
            ...
            '''
            acc_data_ar.append((rt60_125_acc, rt60_250_acc, rt60_500_acc, rt60_1000_acc,rt60_2000_acc, rt60_4000_acc))
            ...
            del rt60_125_acc, rt60_250_acc, rt60_500_acc, rt60_1000_acc, rt60_2000_acc, rt60_4000_acc
            
            #del absorp_125_acc, absorp_250_acc, absorp_500_acc, absorp_1000_acc, absorp_2000_acc, absorp_4000_acc
...
path="/home/psrivastava/baseline/scripts/pre_processing/results_mse_exp/"
for epoch in range(120):
    ar_loss, batch_loss_ar, adcc = train(net, train_dl, optimizer, epoch, ar_loss, batch_loss_ar,mse_loss)
    val_data_ar, acc_data_ar, local_dt_sp = val(net, val_dl, optimizer, epoch, val_data_ar, acc_data_ar,mse_loss)

    np.save(path+"volume_ar_loss.npy", ar_loss)
    np.save(path+"volume_batch_loss_ar.npy", batch_loss_ar)
    np.save(path+"volume_val_data_ar.npy", val_data_ar)
    np.save(path+"volume_acc_data_ar.npy", acc_data_ar)
    #np.save("mlh_bnf_track_" + str(epoch) + "_var_.npy", track_var)

    # save best model
    if epoch == 0:
        save_best_val = val_data_ar[-1]
        np.save(path+"volume_dummy_input_mean_sh.npy", adcc)
    elif save_best_val > val_data_ar[-1]:
        torch.save(
            {'model_dict_1': net_1.state_dict(), 'model_dict_2': net_2.state_dict(), 'model_dict_ens': net.state_dict(),
             'optimizer_dic': optimizer.state_dict(), 'epoch': epoch, 'loss': val_data_ar[-1]},
            path+"volume_tas_save_best_sh.pt")
        save_best_val = val_data_ar[-1]
        np.save(path+"volume_bnf_mag_96ms_" + str(epoch) + ".npy", local_dt_sp)
