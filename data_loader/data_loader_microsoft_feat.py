...
#abcd=h5py.File('/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/noisy_mixture_lwh_2.hdf5','r')
rt60_file=h5py.File('rt60_anno_room_20_2_median.hdf5','r')
anno_file=h5py.File('absorption_surface_calcul_2.hdf5','r')
abcd=h5py.File("/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/microsoft_feat.hdf5",'r')

class binuaral_dataset(Dataset):
    def __init__(self,randomized_arr,root_dir_noisy_mix,root_dir_labels):

        self.random_arr=np.load(randomized_arr)
    ...
    def __getitem__(self, item):
        ...
        bn_sample_vp=abcd['room_nos'][item[0]]['nsmix_f'][(vp-1),:,:]
        #print("from data loader",bn_sample_vp.shape)
        ...
        sample={'bnsample':bn_sample_vp,'surface':surface,'volume':volume,'absorption':absorp,'rt60':rt60,'room':int(item[0].split("_")[1]),'vp':vp}
        return sample
