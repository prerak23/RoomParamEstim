...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar, mse_loss):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        target_=absorption.reshape(128,6)
        ...
        #volume_mse += float(pred(output[:, 1] , volume.view(128)).item())
        ...
        ab_125_ += float(pred(output[:,0], absorption[:, 0]).item())
        ab_250_ += float(pred(output[:,1], absorption[:, 1]).item())
        ab_500_ += float(pred(output[:,2], absorption[:, 2]).item())
        ab_1000_ += float(pred(output[:,3], absorption[:, 3]).item())
        ab_2000_ += float(pred(output[:,4], absorption[:, 4]).item())
        ab_4000_ += float(pred(output[:,5], absorption[:, 5]).item())
        ...
        if epoch == 0:
            adcc = np.concatenate((adcc, absorption.detach().cpu().clone().numpy().reshape(128, 6)), axis=0)
        ...
        if batch_idx % 100 == 99:
            ...
            batch_loss_ar.append((surface_mse / 100, volume_mse / 100, (rt60_125_) / 100, (rt60_250_) / 100,
                                  ...
                                  (ab_4000_) / 100))
            '''
            batch_loss_ar.append(((ab_125_)/100, (ab_250_)/100, (ab_500_)/100 , (ab_1000_)/100 , (ab_2000_)/100 , (ab_4000_)/100))
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar,mse_loss):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        target_1=absorption.reshape(128,6)
        ...
        #volume_loss_mse_t += pred(output[:, 1] , volume.view(128))
        ...
        absorp_loss_mse_125_t += pred(output[:, 0], absorption[:, 0])
        absorp_loss_mse_250_t += pred(output[:, 1], absorption[:, 1])
        absorp_loss_mse_500_t += pred(output[:, 2], absorption[:, 2])
        absorp_loss_mse_1000_t += pred(output[:,3], absorption[:, 3])
        absorp_loss_mse_2000_t += pred(output[:,4], absorption[:, 4])
        absorp_loss_mse_4000_t += pred(output[:,5], absorption[:, 5])
        
        target_ = torch.cat((absorption, rm.reshape(128, 1), vp.reshape(128, 1)),axis=1)  # absorption , rt60
        ...
        #rt60_loss_mse_125_t, rt60_loss_mse_250_t, rt60_loss_mse_500_t, rt60_loss_mse_1000_t, rt60_loss_mse_2000_t, rt60_loss_mse_4000_t
        
        del val_loss_t,absorp_loss_mse_125_t, absorp_loss_mse_250_t, absorp_loss_mse_500_t, absorp_loss_mse_1000_t, absorp_loss_mse_2000_t, absorp_loss_mse_4000_t

        # del val_loss_t,volume_loss_mse_t,surface_loss_mse_t

        if batch_idx % 50 == 49:
            #surface_acc = (surface_loss_mse / 100)

            #volume_acc = (volume_loss_mse / 100)

            '''
            rt60_125_acc = (rt60_loss_mse_125 / 100)

            rt60_250_acc = (rt60_loss_mse_250 / 100)

            rt60_500_acc = (rt60_loss_mse_500 / 100)
            rt60_1000_acc = (rt60_loss_mse_1000 / 100)
            rt60_2000_acc = (rt60_loss_mse_2000 / 100)
            rt60_4000_acc = (rt60_loss_mse_4000 / 100)
            '''
            ...
            acc_data_ar.append((absorp_125_acc, absorp_250_acc, absorp_500_acc, absorp_1000_acc, absorp_2000_acc, absorp_4000_acc))
            ...
            #rt60_125_acc, rt60_250_acc, rt60_500_acc, rt60_1000_acc, rt60_2000_acc, rt60_4000_acc
...
for epoch in range(50):
    ...
    np.save("abs_ar_loss.npy", ar_loss)
    np.save("abs_batch_loss_ar.npy", batch_loss_ar)
    np.save("abs_val_data_ar.npy", val_data_ar)
    np.save("abs_acc_data_ar.npy", acc_data_ar)
    #np.save("mlh_bnf_track_" + str(epoch) + "_var_.npy", track_var)

    # save best model
    if epoch == 0:
        save_best_val = val_data_ar[-1]
        np.save("abs_dummy_input_mean_sh.npy", adcc)
        ...
        torch.save(
            ...
            "abs_tas_save_best_sh.pt")
        save_best_val = val_data_ar[-1]
        np.save("abs_bnf_mag_96ms_" + str(epoch) + ".npy", local_dt_sp)
