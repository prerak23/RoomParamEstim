...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar, mse_loss):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        rt60_125_ += float(pred(output[:, 2], rt60[:, 0]).item())
        rt60_250_ += float(pred(output[:, 3], rt60[:, 1]).item())
        rt60_500_ += float(pred(output[:, 4], rt60[:, 2]).item())
        rt60_1000_ += float(pred(output[:, 5], rt60[:, 3]).item())
        rt60_2000_ += float(pred(output[:, 6], rt60[:, 4]).item())
        rt60_4000_ += float(pred(output[:, 7], rt60[:, 5]).item())
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar,mse_loss):
    ...
    local_data_sp = np.zeros([1, 30])  # 42 #44
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        rt60_loss_mse_125_t += pred(output[:, 2], rt60[:, 0])
        rt60_loss_mse_250_t += pred(output[:, 3], rt60[:, 1])
        rt60_loss_mse_500_t += pred(output[:, 4], rt60[:, 2])
        rt60_loss_mse_1000_t += pred(output[:, 5], rt60[:, 3])
        rt60_loss_mse_2000_t += pred(output[:, 6], rt60[:, 4])
        rt60_loss_mse_4000_t += pred(output[:, 7], rt60[:, 5])
...
for epoch in range(50):
    ...
    np.save("mse_ar_loss_50.npy", ar_loss)
    np.save("mse_batch_loss_ar_50.npy", batch_loss_ar)
    np.save("mse_val_data_ar_50.npy", val_data_ar)
    np.save("mse_acc_data_ar_50.npy", acc_data_ar)
    
    #np.save("mse_bnf_track_" + str(epoch) + "_var_.npy", track_var)

    # save best model
    if epoch == 0:
        save_best_val = val_data_ar[-1]
        np.save("mse_dummy_input_mean_sh.npy", adcc)
        ...
        torch.save(
            ...
            "mse_tas_save_best_sh_50.pt")
        save_best_val = val_data_ar[-1]
        np.save("mse_bnf_mag_96ms_50_" + str(epoch) + ".npy", local_dt_sp)
